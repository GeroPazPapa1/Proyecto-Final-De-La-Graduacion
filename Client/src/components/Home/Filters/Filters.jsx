...
export default function Filters() {
  ...
  const handleChangePrice = async (e) => {
    ...
    const priceActions = {
      ...
      "50000 - 300000": {
        minPrice: checked ? "50000" : "",
        maxPrice: checked ? "300000" : "",
      },
  ...
  useEffect(() => {
    // Si los estados Loadeds son undifined se hace la solicitud para obtener Marcas, Colores, Locacion
  ...
  const handleSubmitPrice = async () => {
    if (
      price.min !== "" &&
      price.max !== "" &&
      Number(price.min) > Number(price.max)
    ) {
      console.log(price);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.column}>
        <button className={styles.buttonCF} onClick={handleChangeClearFilters}>
          CLEAR FILTERS
        </button>
        <h3>State</h3>
        <label>
      ...
      <div className={styles.column}>
        ...
        <input
          ...
          onChange={handleFiltersPrecies}
        />
        <button className={styles.button} onClick={handleSubmitPrice}>
          Search
        </button>
      </div>
      <div className={styles.column}>
      ...
      {brandQuerys.length > 5 && (
        <button onClick={toggleShowAllBrands} className={styles.buttonToggle}>
          {showAllBrands ? (
            <>
              Show less{" "}
              <img className={styles.iconF} src={ICONUP} alt="IconU..." />
            </>
          ) : (
            <>
              Show more{" "}
              <img className={styles.iconF} src={ICONDOWN} alt="IconD..." />
            </>
          )}
        </button>
      )}
